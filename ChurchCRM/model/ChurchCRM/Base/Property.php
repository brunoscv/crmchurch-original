...
abstract class Property implements ActiveRecordInterface
{
    ...
    /**
     * @var        ObjectCollection|ChildRecordProperty[] Collection to store aggregation of ChildRecordProperty objects.
     */
    protected $collRecordProperties;
    protected $collRecordPropertiesPartial;
     ...
     * @var ObjectCollection|ChildRecordProperty[]
     */
    protected $recordPropertiesScheduledForDeletion = null;
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->recordPropertiesScheduledForDeletion !== null) {
                if (!$this->recordPropertiesScheduledForDeletion->isEmpty()) {
                    \ChurchCRM\RecordPropertyQuery::create()
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            ...
            if (null !== $this->collRecordProperties) {
    ...
    {
        if ('RecordProperty' == $relationName) {
     ...
     *
     * @return void
     * @see        addRecordProperties()
     */
    public function clearRecordProperties()
    {
        $this->collRecordProperties = null; // important to set this to NULL since that means it is uninitialized
    }

    /**
     * Reset is the collRecordProperties collection loaded partially.
     */
    public function resetPartialRecordProperties($v = true)
    {
        $this->collRecordPropertiesPartial = $v;
    }
     ...
     * @return void
     */
    public function initRecordProperties($overrideExisting = true)
    {
        if (null !== $this->collRecordProperties && !$overrideExisting) {
            return;
        }

        $collectionClassName = RecordPropertyTableMap::getTableMap()->getCollectionClassName();

        $this->collRecordProperties = new $collectionClassName;
        $this->collRecordProperties->setModel('\ChurchCRM\RecordProperty');
    }

    /**
     * Gets an array of ChildRecordProperty objects which contain a foreign key that references this object.
     *
     ...
     * @param      ConnectionInterface $con optional connection object
     * @return ObjectCollection|ChildRecordProperty[] List of ChildRecordProperty objects
     * @throws PropelException
     */
    public function getRecordProperties(Criteria $criteria = null, ConnectionInterface $con = null)
    {
        $partial = $this->collRecordPropertiesPartial && !$this->isNew();
        if (null === $this->collRecordProperties || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collRecordProperties) {
                ...
                if (null !== $criteria) {
                    if (false !== $this->collRecordPropertiesPartial && count($collRecordProperties)) {
                        $this->initRecordProperties(false);

                        foreach ($collRecordProperties as $obj) {
                            if (false == $this->collRecordProperties->contains($obj)) {
                ...
                if ($partial && $this->collRecordProperties) {
    ...
    /**
     * Sets a collection of ChildRecordProperty objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param      Collection $recordProperties A Propel collection.
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildProperty The current object (for fluent API support)
     */
    public function setRecordProperties(Collection $recordProperties, ConnectionInterface $con = null)
    {
        /** @var ChildRecordProperty[] $recordPropertiesToDelete */
        $recordPropertiesToDelete = $this->getRecordProperties(new Criteria(), $con)->diff($recordProperties);
        ...
        $this->collRecordProperties = $recordProperties;
        $this->collRecordPropertiesPartial = false;
    ...
    /**
     * Returns the number of related RecordProperty objects.
     *
     * @param      Criteria $criteria
     * @param      boolean $distinct
     * @param      ConnectionInterface $con
     * @return int             Count of related RecordProperty objects.
     * @throws PropelException
     */
    public function countRecordProperties(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->collRecordPropertiesPartial && !$this->isNew();
        if (null === $this->collRecordProperties || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collRecordProperties) {
    ...
    /**
     * Method called to associate a ChildRecordProperty object to this object
     * through the ChildRecordProperty foreign key attribute.
     *
     * @param  ChildRecordProperty $l ChildRecordProperty
     * @return $this|\ChurchCRM\Property The current object (for fluent API support)
     */
    public function addRecordProperty(ChildRecordProperty $l)
    {
        if ($this->collRecordProperties === null) {
            $this->initRecordProperties();
            $this->collRecordPropertiesPartial = true;
        ...
        if (!$this->collRecordProperties->contains($l)) {
            $this->doAddRecordProperty($l);

            if ($this->recordPropertiesScheduledForDeletion and $this->recordPropertiesScheduledForDeletion->contains($l)) {
                $this->recordPropertiesScheduledForDeletion->remove($this->recordPropertiesScheduledForDeletion->search($l));
            }
        }
    ...
    /**
     * @param ChildRecordProperty $recordProperty The ChildRecordProperty object to add.
     */
    protected function doAddRecordProperty(ChildRecordProperty $recordProperty)
    {
        $this->collRecordProperties[]= $recordProperty;
        $recordProperty->setProperty($this);
    }

    /**
     * @param  ChildRecordProperty $recordProperty The ChildRecordProperty object to remove.
     * @return $this|ChildProperty The current object (for fluent API support)
     */
    public function removeRecordProperty(ChildRecordProperty $recordProperty)
    {
