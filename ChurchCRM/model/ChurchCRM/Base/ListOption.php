...
abstract class ListOption implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\ChurchCRM\\Map\\ListOptionTableMap';
    ...
    /**
     * Initializes internal state of ChurchCRM\Base\ListOption object.
     * @see applyDefaults()
     */
     ...
     *
     * @return $this|ListOption The current object, for fluid interface
     */
     ...
     * @return $this|\ChurchCRM\ListOption The current object (for fluent API support)
     */
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : ListOptionTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->lst_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : ListOptionTableMap::translateFieldName('OptionId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->lst_optionid = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : ListOptionTableMap::translateFieldName('OptionSequence', TableMap::TYPE_PHPNAME, $indexType)];
            $this->lst_optionsequence = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : ListOptionTableMap::translateFieldName('OptionName', TableMap::TYPE_PHPNAME, $indexType)];
            $this->lst_optionname = (null !== $col) ? (string) $col : null;
            $this->resetModified();
            ...
            return $startcol + 4; // 4 = ListOptionTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\ChurchCRM\\ListOption'), 0, $e);
        }
    }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildListOptionQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\ChurchCRM\ListOption
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\ChurchCRM\ListOption The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ListOptionTableMap::COL_LST_ID)) {
            $criteria->add(ListOptionTableMap::COL_LST_ID, $this->lst_id);
        }
        if ($this->isColumnModified(ListOptionTableMap::COL_LST_OPTIONID)) {
            $criteria->add(ListOptionTableMap::COL_LST_OPTIONID, $this->lst_optionid);
        }
        if ($this->isColumnModified(ListOptionTableMap::COL_LST_OPTIONSEQUENCE)) {
            $criteria->add(ListOptionTableMap::COL_LST_OPTIONSEQUENCE, $this->lst_optionsequence);
        }
        if ($this->isColumnModified(ListOptionTableMap::COL_LST_OPTIONNAME)) {
            $criteria->add(ListOptionTableMap::COL_LST_OPTIONNAME, $this->lst_optionname);
        }
    ...
    {
        $criteria = ChildListOptionQuery::create();
        $criteria->add(ListOptionTableMap::COL_LST_ID, $this->lst_id);
        $criteria->add(ListOptionTableMap::COL_LST_OPTIONID, $this->lst_optionid);
    ...
    {
        return (null === $this->getId()) && (null === $this->getOptionId());
    }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \ChurchCRM\ListOption (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \ChurchCRM\ListOption Clone of current object.
     * @throws PropelException
     */
     ...
     * If this ChildListOption is new, it will return
     ...
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildListOption The current object (for fluent API support)
     */
