...
/**
 * Base class that represents a row from the 'calendar_events' table.
 *
...
abstract class CalendarEvent implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\ChurchCRM\\Map\\CalendarEventTableMap';
    ...
    /**
     * Initializes internal state of ChurchCRM\Base\CalendarEvent object.
     */
     ...
     * @return $this|\ChurchCRM\CalendarEvent The current object (for fluent API support)
     */
    public function setCalendarId($v)
    {
        ...
        if ($this->calendar_id !== $v) {
            $this->calendar_id = $v;
            $this->modifiedColumns[CalendarEventTableMap::COL_CALENDAR_ID] = true;
        }

        if ($this->aCalendar !== null && $this->aCalendar->getId() !== $v) {
            $this->aCalendar = null;
        }
    ...
    public function setEventId($v)
    {
        ...
        if ($this->event_id !== $v) {
            $this->event_id = $v;
            $this->modifiedColumns[CalendarEventTableMap::COL_EVENT_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : CalendarEventTableMap::translateFieldName('CalendarId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->calendar_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : CalendarEventTableMap::translateFieldName('EventId', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 2; // 2 = CalendarEventTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\ChurchCRM\\CalendarEvent'), 0, $e);
        }
    }
    ...
    {
        if ($this->aCalendar !== null && $this->calendar_id !== $this->aCalendar->getId()) {
            $this->aCalendar = null;
        }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(CalendarEventTableMap::COL_CALENDAR_ID)) {
            $modifiedColumns[':p' . $index++]  = 'calendar_id';
        }
        if ($this->isColumnModified(CalendarEventTableMap::COL_EVENT_ID)) {
            $modifiedColumns[':p' . $index++]  = 'event_id';
        }

        $sql = sprintf(
            'INSERT INTO calendar_events (%s) VALUES (%s)',
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\ChurchCRM\CalendarEvent
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\ChurchCRM\CalendarEvent The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(CalendarEventTableMap::COL_CALENDAR_ID)) {
            $criteria->add(CalendarEventTableMap::COL_CALENDAR_ID, $this->calendar_id);
        }
        if ($this->isColumnModified(CalendarEventTableMap::COL_EVENT_ID)) {
            $criteria->add(CalendarEventTableMap::COL_EVENT_ID, $this->event_id);
        }
    ...
    {
        $validPk = null !== $this->getCalendarId() &&
            null !== $this->getEventId();

        $validPrimaryKeyFKs = 2;
        $primaryKeyFKs = [];

        //relation calendar_events_fk_d4a5d4 to table calendars
        if ($this->aCalendar && $hash = spl_object_hash($this->aCalendar)) {
            $primaryKeyFKs[] = $hash;
        } else {
            $validPrimaryKeyFKs = false;
        }

        //relation calendar_events_fk_c63715 to table events_event
    ...
    {
        return (null === $this->getCalendarId()) && (null === $this->getEventId());
    }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \ChurchCRM\CalendarEvent (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \ChurchCRM\CalendarEvent Clone of current object.
     * @throws PropelException
     */
    ...
    {
        if ($this->aCalendar === null && ($this->calendar_id != 0)) {
            $this->aCalendar = ChildCalendarQuery::create()->findPk($this->calendar_id, $con);
            /* The following can be used additionally to
                guarantee the related object contains a reference
                to this object.  This level of coupling may, however, be
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aCalendar->addCalendarEvents($this);
             */
        }
    ...
    public function getEvent(ConnectionInterface $con = null)
    {
        if ($this->aEvent === null && ($this->event_id != 0)) {
            ...
            /* The following can be used additionally to
                ...
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aEvent->addCalendarEvents($this);
             */
        }
    ...
    {
        if (null !== $this->aCalendar) {
            $this->aCalendar->removeCalendarEvent($this);
        }
        if (null !== $this->aEvent) {
            $this->aEvent->removeCalendarEvent($this);
        }
